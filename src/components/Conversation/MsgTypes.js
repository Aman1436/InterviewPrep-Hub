...
import { Stack, Box, Divider, Typography, Link,IconButton ,Menu,MenuItem} from "@mui/material";
...
//Preview of the link, the link and the message with the link
const LinkMsg = ({ el }) => {
    const theme = useTheme();
    return (
        <Stack direction="row" justifyContent={el.incoming ? "start" : "end"}>
    ...
    return (
        <Stack direction="row" justifyContent={el.incoming ? "start" : "end"}>
            <Box p={1.5} sx={{
                ...
                <Stack spacing={2}>
                    <Stack p={2} direction="column" spacing={3} alignitem="center" sx={{ backgroundColor: theme.palette.background.paper, borderRadius: 1 }}>
                        <Typography variant="body2" color={theme.palette.text}>
                            {el.message}
                        </Typography>
                    </Stack>
                    <Typography variant="body2" color={el.incoming ? el.theme.palette.text : "#fff"}>
                        {el.reply}
                    </Typography>
        ...
        <Stack direction="row" justifyContent={el.incoming ? "start" : "end"}>
            <Box p={1.5} sx={{
                ...
                <Stack spacing={1}>
                    ...
                    />
                    <Typography variant="body2" color={el.incoming ? theme.palette.test : "#fff"}>
                        {el.message}
                    </Typography>
                </Stack>
...
const Timeline = ({ el }) => {
    const theme = useTheme();
    return <Stack direction="row" alignItems="center" justifyConten="space-between">
        {/* Adding the timeline bar */}
        <Divider width="46%" />
